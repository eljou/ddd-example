...
export class FileReservationRepository implements ReservationRepository {
  ...
  async search(crit: Criteria<Reservation>): Promise<Reservation[]> {
    const { props: criteria } = crit
    const offset = criteria.offset ?? 0

    const reservations = (await this.dbLines())
      .map(this.strToReservation)
      .filter(r => criteria.filters.reduce((prev, filter) => prev && this.evaluateFilter(r, filter), true))
      .slice(offset, criteria.limit ? offset + criteria.limit : undefined)
